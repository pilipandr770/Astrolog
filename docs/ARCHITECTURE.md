...
3. **Астролог-коуч** (`claude_service.generate_post_report_reply`, состояние
   ...
   `conversation_state.last_interpretation`, не переинтерпретируется заново,
   чтобы не противоречить PDF), помогает применить рекомендации в жизни, с
   элементами коучинга/мотивации — но без выдуманных фактов. Удержание
   клиента: коуч знает точную дату окончания месячного календаря
   (`conversation_state.report_calendar_end_date`) и ненавязчиво (не в
   каждом сообщении) предлагает продлить, когда об этом естественно заходит
   речь или период уже истёк/истекает. При согласии клиента (tool-call
   `start_renewal`) — `dialog_manager._handle_renewal()` НЕ запрашивает дату/
   время/место/стиль заново (они уже сохранены), а сразу создаёт новую
   Stripe-сессию и переводит в `awaiting_payment`; после оплаты
   `report_generator` пересчитывает карту на новый 30-дневный период.
